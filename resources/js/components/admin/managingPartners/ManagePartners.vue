...
<v-container>
    <v-card>
        <v-card-title class="mb-3">
            <span class="mr-3 ml-3">{{$t('managingPartners')}}</span>
            <v-spacer></v-spacer>
            ...
            <v-dialog v-model="dialog" max-width="600px">
                <template v-slot:activator="{ on, attrs }">
                    <v-btn color="primary" class="mt-4" dark v-bind="attrs" v-on="on">
                        <v-icon>mdi-plus</v-icon> {{$t('newPartner')}}
                    </v-btn>
                </template>
...
export default {
    ...
    data() {
        return {
            ...
            headers: [{
                ...
                {
                    text: this.$t('license_number'),
                    value: 'license_number'
                },
                {
                    text: this.$t('license_start_date'),
                    value: 'license_start_date'
                },
                {
                    text: this.$t('license_end_date'),
    ...
    methods: {
        ...
        deleteEmployee() {
            axios.delete(route('admin.employees.destroy', {
                'employee': this.currentEmployeeDelete
            })).then(res => {
                console.log('res', res)
        ...
        fetchPartners() {
            axios.get(route('employee.type', 'شريك اداري')).then(res => {
                console.log('res', res);
                this.employeesArray = res.data.employees;
            })
        }
    },
    filters: {
